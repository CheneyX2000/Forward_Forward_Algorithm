...
class FFLayer:
    """
    FFLayer is a class that implements a single layer in a Forward-Forward Neuron Network

    It trains on layer "goodness".
    This impl use tanh() as activation func, and use Xavier initialization for weights.
    ...
    def __init__(self, input_dim, output_dim, learning_rate=0.1):
        ...
        # use Xavior/Glorot initialization for weights, because we will use tanh as activation func
        self.W = np.random.randn(self.in_dim, self.out_dim) * np.sqrt(2 / self.in_dim)
